...
def get_entry(key, year, db_type)
  ...
  #まずは#{year}.hdbから読み込む
  db.open("#{DATA_PATH}/db/#{year}/#{db_type}.hdb", HDB::OWRITER | HDB::OCREAT)
  value_1 = db.get(key)
  db.close

  #その後#{year}/error.hdbも読み込む
  #valueが存在しない場合nilが返ってくる
  db.open("#{DATA_PATH}/db/#{year}/#{db_type}_error.hdb", HDB::OWRITER | HDB::OCREAT)
  value_2 = db.get(key)
  db.close
...
def put_entry(param)
  ...
  #error_flagがtrueの場合は#{year}/#{db_type}_error.hdbを開く
  if error_flag
    db.open("#{DATA_PATH}/db/#{year}/#{db_type}_error.hdb", HDB::OWRITER | HDB::OCREAT)
  else
    db.open("#{DATA_PATH}/db/#{year}/#{db_type}.hdb", HDB::OWRITER | HDB::OCREAT)    
